...
sudo mv otrs-$VERSION $INSTALL_DIR

echo "Add otrs user..."
sudo useradd -d $INSTALL_DIR/ -c 'OTRS user' otrs
...
sudo chown -R otrs.apache $INSTALL_DIR 

echo "Check perl dependencies"
sudo perl $INSTALL_DIR/bin/otrs.CheckModules.pl
sudo perl -cw $INSTALL_DIR/bin/cgi-bin/index.pl
sudo perl -cw $INSTALL_DIR/bin/cgi-bin/customer.pl
bin/otrs.SetPermissions.pl --web-group=apache
sudo $INSTALL_DIR/bin/otrs.SetPermissions.pl
...
sudo cp /tmp/config.pl $INSTALL_DIR/module-tools/config.pl
